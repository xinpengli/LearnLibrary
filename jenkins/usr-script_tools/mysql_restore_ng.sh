...
##下载数据库备份文件
cd $restore_file
wget -q http://172.16.19.67/mysql_bak/$dir_name/$mysql_back_file
...
rm -rf /tmp/$base_name
###暂时测试使用
#cd /usr/local/geekplus/data/mysql/artemis
#wget http://172.16.19.67/mysql_bak/empty/request_log.ibd
#chown mysql:mysql request_log.ibd

#mysql -uroot -proot  <<EOF
#use artemis;
#ALTER TABLE request_log IMPORT   TABLESPACE;
#EOF
